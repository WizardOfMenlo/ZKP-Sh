...
\begin{example}
    ...
    The following algorithm is a Perfect Zero Knowledge Proof System for \texttt{GI}.
    \begin{enumerate}
    ...
    \begin{proof}
        ...
        We aim to first show that, for $I \in_R \{1, 2\}$, $\Pi \in_R S_{V_2}$ the random variables $\Pi(G_2)$ (sent by the prover) and $\Pi(G_I)$ (computed by the simulator) are identically distributed. 
        This is not too bad, since we have shown before that $\Pr[\Pi(G_1) = G'] = \Pr[\Pi(G_2) = G']$ and as such 

        \begin{align*} 
            \Pr[\Pi(G_I) = G'] & \\
            &= \sum_{i \in \{1, 2\}} \Pr[\Pi(G_I) = G' | I = i]\Pr[I = i] \\
            &= \frac{1}{2}(\Pr[\Pi(G_1) = G'] + \Pr[\Pi(G_2) = G'])  \\
            &= \Pr[\Pi(G_2) = G']
        \end{align*}

        As such $\Pi(G_I)$ is identically distributed as $\Pi(G_2)$, and we are motivated in defining $H = \Pi(G_I) = \Pi(G_2)$.
        Now, given $x, r, H$, $V'$'s output is completely determined, and as such we can call it $v(x, r, H)$.
        We want now to show that \[\Pr[ | v(x,r,H) = I] \]
        \end{proof}
\end{example}
...
\[ =  - 2 ^ {-n} + 2^{-n} \Pr[D(B_n, 1^n) = 1]  \geq - 2^{-n}  \]
