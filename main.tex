...
\begin{abstract}
    ...
    Finally, we will detail some applications of the theory, showing various "flavours" of ZKP for the Discrete Logarithm problem that will culminate in
...
selecting "Run all Cells" or by individually stepping on each Cell and pressing Shift+Enter. Each notebook is complete
...
\footnote{The solution I found is to use the hexdigest of an hash function, prepend it with the \texttt{0x} prefix
    and then let python parse it into an integer, which is less than ideal}
...
\footnote{In fact I use this in the Graph Non Isomorphism protocol, as finding isomorphisms for a
...
the bit committed value before the reveal phase.
