...
\begin{abstract}
    ...
    building up from Interactive Proofs to Perfect Zero Knowledge Proofs. In the process, we will have shown interactive
    proofs for Graph Non Isomorphism (a co-$\npol$ problem), and zero knowledge proofs for both Graph Isomorphism and Graph 3-Colorability.
    Finally, we will have detailed some applications of the theory, showing various "flavours" of ZKP for the Discrete Logarithm problem that will culminate in
    Schnorr Signature. We will compound this with the Secure Remote Protocol, a protocol that uses a similar ZKP in order
...
The first and stricter class of zero knowledge proofs are the so called perfect zero knowledge proofs. Ideally, according to the above definition we would like the definition of zero knowledge proof to simply require that the ensembles $\left<P, V\right>(x)$ and $M_{V'}(x)$ are identically distributed. However, as far as we know, no non trivial\footnote{Here, trivial means in $\bpp$, as every language in that class trivially satisfies the definition} languages satisfy that requirement. The underlying problem is that often the simulator is not able to answer the queries of the verifier and needs to be able to give up. Using this we can give the following definition of Perfect Zero Knowledge Proof.
\begin{definition}
...
Once the second requests the opening of the seal, the first party sends a certificate that, together with the transcript, reveals the commited value and proves that it was indeed commited.
...
In particular, note that once $S$ has commited a value, in order to provide verification, it can send its private input and the random coin it used to $R$, which can then
...
For a more through description, look no further than the appendix.
...
Instead, we can use the stronger assumption of the existence of one way permutations (i.e. one way functions that are also bijections), to result in a simpler proof.\par
...
\begin{theorem}
    Let $f: \bin^* \to \bin^*$ be a one-way permutation. Let $b: \bin^* \to \bin$ be a hard-core of $f$. Then the following is a bit commitment scheme.
    \begin{itemize}
...
\begin{example}
    ...
    This graph is 3-colorable, with coloring $\pi$, which is unknown to the verifier.
    \footnote{As always, such $\pi$ could be either computed by a superior prover, or given as auxiliary input}
    \begin{enumerate}
...
\begin{itemize}
    \item The prover has an $x$ such that $\alpha^x \equiv \beta \pmod N$. The conditions are as above.
...
in which the user communicates a username and a password to a server, which is considered trusted and that
...
It turns out \cite{goldreichFoundationsCryptographyVol2007} that the strongly one way functions exist if and only if weakly one way functions do. This justifies our assuming only the existence of vague one way functions, without specifying the type.
...
is not only hard to solve in the worst case, but actually seems to be extremely complicated to solve in the average\footnote{It can be shown
    using random self reducibility, that if a hard instance exists than on the average the problem will also be hard} (CITE).
